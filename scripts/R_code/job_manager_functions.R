...
# changed to new tracking apporach, commit "tracked new object info through tracking code, debugged"
# see git for original verison of following code
...
track_objects <- function(main_path,
  ...
  cat(groovy_params)
  system2('/home/james/software/Fiji.app/ImageJ-linux64',c("--ij2", "--headless", "--allow-multiple", "--mem=20000M", "--run", "/home/james/work/eclipse-workspace/Segmented_Image_Analysis/scripts/get_tracks_parameterised.groovy",paste0("'",groovy_params,"'")),wait=wait)
  end_time = Sys.time()
...
track_objects_lookup <- function(datasetName,segName,lu,saveName="trackNodeTable.csv",objectFolder="objectAnalysis"){
  r = which(lu$name==datasetName)
  pth = paste0(vis_rt,datasetName,"/",segName,"/",objectFolder,"/")
  track_objects(pth,paste0(pth,saveName))
}
...
convert_images_for_visualiser <- function(imageFolder,segFolder,probMapFolder,imageSaveFolder,segSaveFolder,probMapSaveFolder,stackNums,cropBox,intensityRange,
  ...
  cat(groovy_params)
  system2('/home/james/software/Fiji.app/ImageJ-linux64',c("--ij2", "--headless", "--allow-multiple", "--run", "/home/james/work/eclipse-workspace/Segmented_Image_Analysis/scripts/export_to_vis.groovy",paste0("'",groovy_params,"'")),wait=wait)
}
...
convert_images_for_visualiser_short <- function(source_fldr,dest_fldr,timeSteps,cropBox,maxIntensity,convertRawSegProb=c(TRUE,TRUE,TRUE),
                                                segName="d18_intAdj_rep1ds1gd_rf",wait=FALSE){
  rdmFldr <- "/home/james/mnt/rdm_adam/MachineLearning_ARC_Grant_work/"
  convert_images_for_visualiser(paste0(rdmFldr,source_fldr,"Decon_output/"),
                                paste0(rdmFldr,source_fldr,"segmentation/",segName,"/segmented/"),
                                paste0(rdmFldr,source_fldr,"segmentation/",segName,"/probability_maps/"),
...
segment_stacks <- function(dataset_name,stackNums,input_path,output_path,sess,
                           ...
                           numberThreadsToUse=3,cpusRequested=3,saveProbabilityMaps=TRUE,prefix='c1-',
...
segment_stacks_lookup <- function(dataset_name,stackNums,lu,sess,ram_GB=80,walltime="1:00:00",
                                  rt_fldr=rdm_rt,modelName='d18_rep1ds1gd_rf',segName="d18_intAdj_rep1ds1gd_rf",jobname_extra=""){
  r = which(lu$name==dataset_name)
  segment_stacks(dataset_name,stackNums,
                 paste0(rt_fldr,lu$path[r],"Decon_output/"),
                 ...
                 modelName=modelName,jobname_extra=jobname_extra,
...
segment_multi_lookup <- function(dataset_name,stackNums,lu,sess,numberStacksPerJob=10,ram_GB=80,walltime="3:00:00",
                                  rt_fldr=rdm_rt,modelName='d19_rep1ds1gd_rf',segName="d19_intAdj_rep1ds1gd_rf",jobname_extra=""){
  r = which(lu$name==dataset_name)
  segment_multi(dataset_name,stackNums,
                 ...
                 sess,
                 lu$intensityScalingFactor[r],lu$intensityScalingFactorTimeStep[r],paste0(lu$cropBox[[r]],collapse=","),
                 numberStacksPerJob=numberStacksPerJob,modelName=modelName,jobname_extra=jobname_extra,
                 ram_GB=ram_GB,walltime=walltime)
}
...
run_object_analysis_lookup <- function(dataset_name,jobNums,lu,sess,segName,objectAnalysis_folder="objectAnalysis",jobname_extra="", rt_fldr=rdm_rt,
                                       dynamic=c(200,20,20,20),minVoxExtraObjects=75,channelsForDistanceMap=c(1,3),smoothingErosionForDistanceMap=c(300,20),
  ...
  run_object_analysis(paste0("objects_",dataset_name,jobname_extra),
                      jobNums,
                      paste0(rt_fldr,lu$path[r],"segmentation/",segName,"/segmented/"),
                      paste0(rt_fldr,lu$path[r],"Decon_output/"),
                      paste0(rt_fldr,lu$path[r],"segmentation/",segName,"/probability_maps/"),
                      ...
                      dynamic=dynamic,minVoxExtraObjects=minVoxExtraObjects)
}
...
run_mesh_generation <- function(jobName,jobNums,imageStackLocation,savePath,sess,
                                ...
                                numberThreadsToUse=2,stackNumberPrefix="-t",stackNumberSuffix="-e",fileNamePrefix="c1-",
                                overwriteExisting=FALSE,targetMeshVertexReduction=c(0.96,0.0,0.8,0.8),meshSmoothingIterations=c(0,0,0,0)){
...
run_mesh_generation_lookup <- function(dataset_name,jobNums,lu,sess,segName,jobname_extra="", rt_fldr=rdm_rt,
                                       ...
                                       numberStacksPerJob=10,ram_GB=10,walltime="5:00:00",
                                       numberThreadsToUse=2,stackNumberPrefix="-t",stackNumberSuffix="-e",fileNamePrefix="c1-",
                                       overwriteExisting=FALSE,verbose=FALSE,targetMeshVertexReduction=c(0.96,0.0,0.8,0.8),meshSmoothingIterations=c(0,0,0,0)){
  ...
  run_mesh_generation(paste0("meshes_",dataset_name,"_",segName,jobname_extra),
                      ...
                      overwriteExisting,targetMeshVertexReduction,meshSmoothingIterations)
}
...
run_skeleton_generation <- function(jobName,jobNums,imageStackLocation,savePath,sess,
                                ...
                                numberThreadsToUse=2,stackNumberPrefix="-t",stackNumberSuffix="-e",fileNamePrefix="c1-",
                                overwriteExisting=FALSE,verbose=FALSE){
